package com.facturacionapi.auth;
...
/*    JWT tiene una firma al final que, depende el algoritmo, es o un secret o una public/private key. Si es HS256 es un secret
y si es RS256 es la public/private key. Public para verificar que no se modifico, y private es la que autentica.
  Para asignar el mac (o la clave RSA), si no hago nada se genera automaticamente. Detecta automaticamente si es un MAC o si es un
RSA. Si arranca con ---BEGIN es un RSA. Con el JwtAccessTokenCOnverter, puedo setear la SigningKey yo a mano.*/

/*
 Para generar el token en postman. con la URL /oauth/token -> en body tengo que poner username, password y grant_type del usuario. Grant
 type es password. TTodo en formato x-www-form-urlencoded.
En Authoriation tengo que poner TYPE: Basic Auth,  el username y el password son los que yo setie en la aplicaicon.EN la clase
authorization serverCOnfig, el metodo Configure puse Username: angularapp y pass: 12345 . Apreto en preview request.
VOy a header y en KEY Athorizaton es Basic, y en Content-Type es aplication/x-www-form-urlencoded.
Si enviamos la solicitud, esto nos devuelve el token con la provia llave. Ahora se puede usar para autenticarse en los endpoints
que no son pulbicos. Ahora para usarlo, en Authorization, TYPE: Bearer  y pego el token entero. Ya se podria hacer la solicitud normlamente.
 */
